...
import { staffGetPointWithdrawalsRequest } from '@/lib/api';
...
export default function App(): JSX.Element {
  ...
  const [alert, setAlert] = useState<IAlertProps>({
    message: 'Getting requested point withdrawals ...',
    isLoading: true,
  });
  ...
  const getRecyclesHandler = async () => {
    ...
    try {
      const response = await staffGetPointWithdrawalsRequest(session?.user.accessToken as string);
      if (response.success) {
        ...
        setAlert({
          message: `Total requested point withdrawals: ${response.success.data.length}`,
        });
      }
    } catch (error: any) {
      setAlert({
