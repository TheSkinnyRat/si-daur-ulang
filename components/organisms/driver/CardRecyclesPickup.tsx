...
import { driverGetRecyclesPickup, driverUpdateRecycle } from '@/lib/api';
...
export default function App(): JSX.Element {
  ...
  const [alert, setAlert] = useState<IAlertProps>({
    message: 'Getting pickup recycles ...',
    isLoading: true,
  });
  ...
  const getRecyclesHandler = async () => {
    ...
    try {
      const response = await driverGetRecyclesPickup(session?.user.accessToken as string);
      if (response.success) {
        ...
        setAlert({
          message: `Total pickup recycles: ${response.success.data.length}`,
        });
      }
    } catch (error: any) {
      setAlert({
